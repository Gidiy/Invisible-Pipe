...
def animate(frame):
    ...
    global location_x
    global distance
    arduino_data_file = f"D:/snake_wall/reading_and_display_wall_snake/{file_name}"  # file_data_location_contain_only_angels
        ...
        with open(arduino_data_file, "r") as file:
            ...
            for line in lines:
                ...
                try:
                    ...
                    plot_vector(
                        vector(degrees_to_radians(-yaw), degrees_to_radians(pitch), degrees_to_radians(roll), distance),
                        color='y')

                    # data of angle's axis -> convert degrees to radians->take 3 angles in radians(Spherical
                    # coordinate system)  and length and return cartesian vector from previous point to the next
                    # point then drew it in the 3D graph and update the last point
